...
const FiveYearDifficulties = ({ courseFiveYearData, courseData }) => {
  ...
  const fiveYearData = data.filter((entry) => 2018 <= parseInt(entry.year) && 
    parseInt(entry.year) <= 2022
  );

  const clickedCourseData = fiveYearData.filter((element) => element.course.id === clickedCourse);

  clickedCourseData.sort((a, b) => (a.year > b.year ? 1 : -1)); // sorts data increasing by year
